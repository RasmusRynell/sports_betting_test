...
def pred(file_name, pred, gamePk):    
    ...
    for i in range(2):
        ...
        model = LogisticRegression(random_state=912, max_iter=5000)

        # cross validation
        f_scores_test = cross_val_score(model, X_all, Y_all, scoring="f1", cv=5)
        f_test_error = round(f_scores_test.mean(),3)
        f_test_std = round(f_scores_test.std(), 3)

        scores_test = cross_val_score(model, X_all, Y_all, cv=5)
        ...
        Y_pred = model.predict_proba(pred_this_game)
...
print(pred("./data/td/pp_victor_hedman.csv", 2.5, 2020020679))
