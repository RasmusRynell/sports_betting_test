                        ...
                        if bet[f"kelly_{o_u}"] > settings['kelly_low_lim'] and\
                            ...
                            if bet[f"proba_{o_u}"] > settings['proba_low_lim'] and\
                                ...
                                if (bet['answer'] == (o_u == "over")):
                                    ...
                                    result['total_bets_won'] += 1 # Stats
                            
                                result['total_bets_betted'] += 1 # Stats

                        result['total_bets'] += 1 # Stats
                        result['data']['bets_return'].append(info['end']) # Stats

                result['data']['days_return'].append(info['end']) # Stats
                result['data']['days'].append(\
                    result['data']['days'][-1] + len(info['bets']*len(settings['bet_on'])) if
                    len(result['data']['days']) > 0 else len(info['bets']*len(settings['bet_on']))) # Stats
