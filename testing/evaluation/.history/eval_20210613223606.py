...
class daily:
    ...
    def eval(self, settings = None):
        ...
        if internal_settings['type'] == "nhl_SOG":
            for csv in internal_settings['csvs']:
                ...
                for outcome in internal_settings['bet_on']:
                df[f'kelly_under'] = ((df[f'odds_under'] - 1) * df[f'proba_under'] - df[f'proba_over']) / (df[f'odds_under'] - 1)
                df[f'kelly_over'] = ((df[f'odds_over'] - 1) * df[f'proba_over'] - df[f'proba_under']) / (df[f'odds_over'] - 1)

                result[f'{csv}-{kelly_risk}'] = self.run_nhl_SOG(df, kelly_risk, csv, internal_settings)
            ...
            for csv in internal_settings['csvs']:
                ...
                result[f'{csv}-{kelly_risk}'] = self.CL_HomeXAway(df, kelly_risk, csv, internal_settings)
    ...
    def run_nhl_SOG(self, df, kelly_risk, csv, settings):
        if settings['rev_data']:
            df = df[::-1].reset_index()
